...
int main ( int argc, char** argv ) {
   ...
   map_init(map,50);
    ...
    while (src.elem[i] != '\\' && src.elem[i] != '/') {
        if (!List_exist(&born,src.elem[i])) ListInsert(&born,j++,src.elem[i]);
        i++;
    }
    j = 1;
    i++;
    while(src.elem[i] != '\n' && src.elem[i] != ' ' && i < 18) {
        if (!List_exist(&surv,src.elem[i])) ListInsert(&surv,j++,src.elem[i]);
        i++;
    }
    fclose(fp);
...
void pop_copy(int pop[],int map[][NB_BLOCK_HEIGHT]){
    int i;
    for (i=0;i<NB_BLOCK_WIDTH-1;i++) pop[i]=pop[i+1];
